...
import {
  JupyterHubConnection,
  JupyterHubConnectionInterface,
} from '../jupyterHub/jupyterHubConnection';
...
describe('JupyterHubConnection', () => {
  ...
  describe('getFileContent', () => {
    ...
    it('should throw an error when not connected', async () => {
      connection['isConnected'] = false;
      await expect(connection.getFileContent('/test.txt')).rejects.toThrow(
        'Not connected to JupyterHub server'
      );
    });
  ...
  describe('createItem', () => {
    ...
    it('should throw an error for unsupported item types', async () => {
      await expect(connection.createItem('/test', 'unknown' as any)).rejects.toThrow(
  ...
  describe('getChildren', () => {
    it('should return children items for a directory', async () => {
      ...
      const mockListContents = jest.fn((pathParam: string) =>
        Promise.resolve([
          {
            type: 'directory',
            name: 'subdir',
      ...
      const expectedChildren = ['/test-dir/subdir', '/test-dir/file.txt'];
