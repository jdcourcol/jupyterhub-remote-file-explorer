...
export class ConnectionManager {
    ...
    public async connect(): Promise<boolean> {
        ...
        if (connections.length > 0) {
            const quickPickItems: Array<vscode.QuickPickItem & { profile?: ConnectionProfile }> = connections.map(profile => ({
                label: profile.name,
            ...
            if (lastConnection) {
                activeItem = quickPickItems.find(item => 
                    item.profile && item.profile.id === lastConnection.id
                );
            }
        ...
        if (connected) {
            vscode.window.showInformationMessage(`Connected to JupyterHub server: ${serverUrl}`);
    ...
    public disconnect(): void {
        if (this._connection) {
            ...
            this._connection = undefined;
            vscode.window.showInformationMessage('Disconnected from JupyterHub server');
    ...
    public async forgetCredentials(): Promise<void> {
        try {
            ...
            vscode.window.showInformationMessage('JupyterHub credentials have been removed');
        } catch (error) {
            console.error('Error removing credentials:', error);
            vscode.window.showErrorMessage(`Failed to remove credentials: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    public async manageConnections(): Promise<void> {
        ...
        if (action.label === 'Connect') {
            ...
            await this.context.globalState.update(ConnectionManager.LAST_CONNECTION_ID, selectedItem.profile.id);
            ...
            vscode.window.showInformationMessage(`Connection '${selectedItem.label}' has been removed.`);
        }
    }
    ...
    public showConnectionStatus(): void {
        if (this.isConnected()) {
            const info = this.getConnectionInfo();
            const connectedMessage = `Connected to JupyterHub server: ${info?.serverUrl} (${info?.username})`;
            
            vscode.window.showInformationMessage(connectedMessage, 'Update Credentials', 'Disconnect')
                .then(selection => {
                    if (selection === 'Update Credentials') {
                        ...
                        vscode.commands.executeCommand('jupyterhub-remote-file-explorer.disconnect');
                    }
                });
        } else {
            vscode.window.showInformationMessage('Not connected to any JupyterHub server.', 'Connect')
                .then(selection => {
